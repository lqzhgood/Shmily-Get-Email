...
let QuotedPrintable = require('@vlasky/quoted-printable');

//Default character set
let defaultCharset = 'utf-8'; //to use if charset=... is missing
...
var emlformat = {
    ...
    getEmailAddress(raw) {
        ...
        let parts = raw.split(/,(?=(?:(?:[^"]*"){2})*[^"]*$)/); //Ignore ',' within the double quotes
        ...
        for (let i = 0; i < parts.length; i++) {
            ...
            if (match) {
                let name = emlformat.unquoteString(match[1]).replace(/"/g, "").trim();
                if (name && name.length) {
                    address.name = name;
                }
    ...
    unquoteString(s, log) {
        ...
        // if (log) console.log('match', match);
        if (match) {
            let charset = getCharsetName(match[1] || defaultCharset); //eq. match[1] = 'iso-8859-2'; charset = 'iso88592'
    ...
    unquotePrintable(s, charset) {
        //Convert =0D to '\r', =20 to ' ', etc.
        // if (!charset || charset == "utf8" || charset == "utf-8") {
        //     return s
        //         .replace(/[=]\r?\n/gi, "") //Join line
        //         .replace(/[=]([\w\d]{2})=([\w\d]{2})=([\w\d]{2})/gi, (matcher, p1, p2, p3, offset, string) => { return Buffer.from([parseInt(p1, 16), parseInt(p2, 16), parseInt(p3, 16)]).toString("utf8"); }).replace(/[=]([\w\d]{2})=([\w\d]{2})=([\w\d]{2})/gi, (matcher, p1, p2, p3, offset, string) => { return Buffer.from([parseInt(p1, 16), parseInt(p2, 16), parseInt(p3, 16)]).toString("utf8"); })
        //         .replace(/[=]([\w\d]{2})=([\w\d]{2})/gi, (matcher, p1, p2, offset, string) => { return Buffer.from([parseInt(p1, 16), parseInt(p2, 16)]).toString("utf8"); }).replace(/[=]([\w\d]{2})=([\w\d]{2})/gi, (matcher, p1, p2, offset, string) => { return Buffer.from([parseInt(p1, 16), parseInt(p2, 16)]).toString("utf8"); })
        //         .replace(/[=]([\w\d]{2})/gi, (matcher, p1, offset, string) => { return String.fromCharCode(parseInt(p1, 16)); }).replace(/[=]([\w\d]{2})/gi, (matcher, p1, offset, string) => { return String.fromCharCode(parseInt(p1, 16)); });
        // }
        // else {
        //     return s
        //         .replace(/[=]([\w\d]{2})=([\w\d]{2})/gi, (matcher, p1, p2, offset, string) => { return iconv.decode(Buffer.from([parseInt(p1, 16), parseInt(p2, 16)]), charset); })
        //         .replace(/[=]([\w\d]{2})/gi, (matcher, p1, offset, string) => { return iconv.decode(Buffer.from([parseInt(p1, 16)]), charset); })
        //         .replace(/[=]\r?\n/gi, ""); //Join line
        // }


        const bufferEncoded = Buffer.from(s, 'ascii');
        const buffer = QuotedPrintable.decode(bufferEncoded, { qEncoding: false });
        const ds = iconv.decode(buffer, charset);
        return ds;
    },
};
...
emlformat.unpack = function (eml, directory, options, callback) {
    ...
    if (typeof eml == "string" || (typeof eml == "object" && eml.headers && eml.body)) {
        emlformat.parse(eml, (error, parsed) => {
            if (error) return callback(error);

            //Save parsed EML as JSON file
            if (options && options.parsedJsonFile) {
                let file = path.resolve(directory, options.parsedJsonFile);
            ...
            emlformat.read(parsed, (error, data) => {
                if (error) return callback(error);

                //Save read structure as JSON file
                if (options && options.readJsonFile) {
                    let file = path.resolve(directory, options.readJsonFile);
                    let dir = path.dirname(file);
                    if (!fs.existsSync(dir)) {
                        fs.mkdirSync(dir);
                    }
                    result.files.push(options.readJsonFile);
                    let json = data.attachments ? JSON.stringify(data) : JSON.stringify(data, " ", 2); //Attachments may be large, so make a compact JSON string
...
emlformat.build = function (data, options, callback) {
    ...
    try {
        ...
        if (typeof data.headers["Content-Type"] == "undefined") {
            ...
            let name = emlformat.getBoundary(data.headers["Content-Type"]);
            if (name) {
                boundary = name;
            }
        }
        ...
        //Build headers
        let keys = Object.keys(data.headers);
        for (var i = 0; i < keys.length; i++) {
            let key = keys[i];
            let value = data.headers[key];
...
emlformat.read = function (eml, options, callback) {
    ...
    function _read(data) {
        try {
            ...
            if (data.headers.Subject) {
                // console.log('data.headers.Subject', data.headers.Subject);
                result.subject = emlformat.unquoteString(data.headers.Subject, true);
            }
            ...
            function _append(headers, content, innerBoundary) {
                let contentType = headers["Content-Type"];
                let charset = getCharsetName(emlformat.getCharset(contentType) || defaultCharset);
                let encoding = headers["Content-Transfer-Encoding"] || result.headers['Content-Transfer-Encoding'] || '';
                if (typeof encoding == "string") {
                    encoding = encoding.toLowerCase();
                }
                // console.log('headers', headers, encoding, charset, contentType);
                if (encoding == "base64") {
                    if (/gbk/i.test(contentType) || /gb2312/i.test(contentType) || /gb18030/i.test(contentType)) {
                        content = Buffer.from(iconv.decode(Buffer.from(content, 'base64'), 'gbk'), 'utf8');
                    }
                    else {
                    ...
                    // content = iconv.decode(Buffer.from(content, 'binary'), 'utf-8');
                }

                const innerBoundaryReg = new RegExp(`--${innerBoundary}--(\\s{0,})$`);

                if (!result.html && contentType && contentType.indexOf("text/html") >= 0) {

                    if (typeof content != "string") {
                        content = iconv.decode(Buffer.from(content), 'utf-8');
                    }
                    //Message in HTML format
                    result.html = content.replace(innerBoundaryReg, '');
                }
                    ...
                    //Plain text message
                    result.text = content.replace(innerBoundaryReg, '');
                }
                else {
                    //Get the attachment
                    ...
                    let id = headers["Content-ID"];
                    if (id) {
                        attachment.id = id;
                    }

                    let name = headers["Content-Disposition"] || headers["Content-Type"];
                    if (name) {
                        let match = /name="?([\s\S]+?)"?((;[\s\S]*)|$)/gi.exec(name);
                        if (match) {
                    ...
                    if (name) {
                        attachment.name = emlformat.unquoteString(name);
                    }
                ...
                for (let i = 0; i < data.body.length; i++) {
                    ...
                    if (typeof b.part == "undefined") {
                        ...
                        if (typeof b.part.body == "undefined") {
                            ...
                            let innerBoundary = b.boundary;

                            _append(headers, content, innerBoundary);
                        }
                        else {
                            for (let j = 0; j < b.part.body.length; j++) {
                                ...
                                let innerBoundary = b.part.body[j].boundary;
    ...
    if (typeof eml == "string") {
        emlformat.parse(eml, options, (error, data) => {
            if (error) return callback(error);
    ...
    for (let i = start; i < lines.length; i++) {
        ...
        if (!insideBody) {
            ...
            if (line == "") {
                ...
                if (ct && /^multipart\//g.test(ct)) {
                    let b = emlformat.getBoundary(ct);
                    if (b && b.length) {
                        findBoundary = b;
                        isMultipart = true;
                        parent.body = [];
